...
public class ProxyChooser {
    ...
    public static String[] chooseproxy(Set<String> havechoice)
    {
        String minn = "";
        int mint = 1000000;
        for (String element: proxymap.keySet()){
            int c = proxymap.get(element);
            if (c < mint && !havechoice.contains(element)){
                minn = element;
                mint = c;
            }
        }
    ...
    public static void getnewproxy() {
        String index_url= "http://svip.kuaidaili.com/api/getproxy/?orderid=910263180614597&num=200&b_pcchrome=1&b_pcie=1&b_pcff=1&b_android=1&b_iphone=1&b_ipad=1&protocol=1&method=2&an_an=1&an_ha=1&sep=1";
