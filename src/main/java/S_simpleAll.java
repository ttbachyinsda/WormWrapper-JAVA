 ...
 */
public class S_simpleAll extends Servant{
    ...
    Document run() {
        ...
        while (true){
            try{
                ...
                Document doc = new Document("timestamp", ts.toString()).append("ykid", ykid).append("method", "simpleall")
                ...
                if (try_num >= max_num+1) {
                    ...
                    Document doc = new Document("timestamp", ts.toString()).append("ykid", ykid)
                            .append("method", "simpleall").append("result", result.trim());
                    collection.insertOne(doc, new SingleResultCallback<Void>() {
                        @Override
                        public void onResult(Void aVoid, Throwable throwable) {
                            Main.havesent ++;
